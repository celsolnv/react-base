...
type TLoadingState = { messageIndex: number; progress: number };
type TLoadingAction =
  | { type: "RESET" }
...
function loadingReducer(
  ...
  switch (action.type) {
    case "RESET":
      return { messageIndex: 0, progress: 0 };
    case "SET_PROGRESS":
      return { ...state, progress: action.payload };
    case "NEXT_MESSAGE":
      return {
        ...state,
        messageIndex: (state.messageIndex + 1) % LOADING_MESSAGES.length,
      };
    default:
      return state;
  }
...
const LoadingScreen = () => {
  const [{ messageIndex, progress }, dispatch] = useReducer(loadingReducer, {
    messageIndex: 0,
    progress: 0,
  });
    ...
    <LazyMotion features={domAnimation}>
      ...
      >
        ...
        >
          ...
          >
            <m.div className="bg-success h-1.5 w-1.5 animate-pulse rounded-full shadow-[0_0_4px_hsl(var(--success)/0.5)]" />
